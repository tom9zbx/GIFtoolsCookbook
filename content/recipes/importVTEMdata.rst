...
With Step 1 complete, the GUI shows the number of data types selected. For our example, that's 2 data groups. Select the first data type to specify from the drop down menu. It will be called "Data type 1". Click the "Specify type" button and the following window will appear:
...
When everything is selected as desired, click OK to load in the time-domain EM data into GIFtools. Depending on the size of the file, this may take some time!
